...
import com.example.capstonewms.mainDevice.WaitingFragment;
...
//onCreate()에서 확인용으로 썼던거
/*final Timer timer = new Timer();

           TimerTask timeTest = new TimerTask() {
               @Override
               public void run() {
                   //Toast 자체가 스레드 위에서 실행되기 때문에 현재 Activity 하위의 runOnUiThread 객체에서 스레드 실행 시켜야함

                   mainActivity.runOnUiThread(new Runnable() {
                       @Override
                       public void run() {
                           Toast.makeText(getApplicationContext(), "Checked Number : " + check, Toast.LENGTH_SHORT).show();
                       }
                   });
                   check++;
               }
           };
        timer.schedule(timeTest, 0, 2000);*/